...
impl From<&EntrySealedCommitted> for Option<AccountPolicy> {
    fn from(val: &EntrySealedCommitted) -> Self {
        ...
        let allow_primary_cred_fallback = val
            .get_ava_single_bool(Attribute::AllowPrimaryCredentialFallback);
...
impl ResolvedAccountPolicy {
        ...
        I: Iterator<Item = AccountPolicy>,
    {
        ...
        let mut accumulate = ResolvedAccountPolicy {
            ...
            allow_primary_cred_fallback: true,
        };
        ...
        iter.for_each(|acc_pol| {
            ...
            if let Some(fallback_to_primary_cred) = acc_pol.fallback_to_primary_cred {
                accumulate.allow_primary_cred_fallback = match accumulate.allow_primary_cred_fallback {
                    Some(acc_fallback) => Some(fallback_to_primary_cred && acc_fallback),
                    None => Some(fallback_to_primary_cred),
                };
            }
        });
